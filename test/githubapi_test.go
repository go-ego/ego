...
func githubConfigRouter(router *ego.Engine) {
	for _, route := range githubAPI {
		router.Handle(route.method, route.path, func(c *ego.Context) {
...
func exampleFromPath(path string) (string, ego.Params) {
	output := new(bytes.Buffer)
	params := make(ego.Params, 0, 6)
	start := -1
	for i, c := range path {
		if c == ':' {
			start = i + 1
		...
		if start >= 0 {
			if c == '/' {
				...
				params = append(params, ego.Param{
					Key:   path[start:i],
					Value: value,
				})
